...
public class SVY21 {
  ...
  private static double calcM(double lat) {          // M: meridian distance.
    ...
    double m =
        a * ((A0 * latR) - (A2 * Math.sin(2 * latR)) + (A4 * Math.sin(4 * latR)) - (A6 * Math.sin(
            6 * latR)));
    return m;
  }
  ...
  private static double calcV(
      double sin2Lat) {        // v: radius of curvature in the prime vertical.
  ...
  public static LatLonCoordinate computeLatLon(double N, double E) {
    ...
    double latTerm2 =
        latFactor * ((Eprime * x3) / 24) * ((-4 * psiPrime2 + (9 * psiPrime) * (1 - tPrime2) + (12
            * tPrime2)));
    ...
    double lonTerm3 =
        ((x5 * secLatPrime) / 120) * ((-4 * psiPrime3) * (1 - 6 * tPrime2) + psiPrime2 * (9
            - 68 * tPrime2) + 72 * psiPrime * tPrime2 + 24 * tPrime4);
    double lonTerm4 =
   ...
   * accessible with .getLatitude(), and longitude, accessible with .getLongitude().
   * <p>
   * This method is a shorthand for the functionally identical public LatLonCoordinate
   * computeLatLon(double N, double E).
   *
  ...
  public static SVY21Coordinate computeSVY21(double lat, double lon) {
    ...
    double nTerm3 = w6 / 720 * v * sinLat * cos5Lat * (
        (8 * psi4) * (11 - 24 * t2) - (28 * psi3) * (1 - 6 * t2) + psi2 * (1 - 32 * t2)
            - psi * 2 * t2 + t4);
